...
System inv(size_t i, const hml_formula &f) {
    return {Sol_MAX, hml_formula::AND(f, hml_formula::LBOX({true}, hml_formula::REC(i)))};
}

System pos(size_t i, const hml_formula& f) {
    return {Sol_MIN, hml_formula::OR(f, hml_formula::LDIAM({true}, hml_formula::REC(i)))};
}


System even(size_t i, const hml_formula& f) {
    return {Sol_MIN, hml_formula::OR(f, hml_formula::AND(hml_formula::LDIAM({true}, hml_formula{}),
                                                         hml_formula::LBOX({true}, hml_formula::REC(i))))};
}

System safe(size_t i, const hml_formula& f) {
    return {Sol_MAX, hml_formula::AND(f, hml_formula::OR(hml_formula::LBOX({true}, hml_formula::FALSEHOOD()),
                                                         hml_formula::LDIAM({true}, hml_formula::REC(i))))};
}
